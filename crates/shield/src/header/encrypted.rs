...
impl Encrypted {
	...
	pub(super) fn encrypt(
		...
		let tag = cipher::Cipher::new(key.as_bytes()).encrypt(&mut bytes, &[]);
		Self { bytes, tag }
	}
	...
	pub(crate) fn decrypt(
		...
		cipher::Cipher::new(key.as_bytes()).decrypt(
			&mut bytes,
			&[],
			self.tag,
		)?;
...
mod tests {
	...
	fn test_encrypt_decrypt() -> Result<(), super::DecryptError> {
		...
		let encrypted = super::Encrypted::encrypt(&header_key, &header);
		assert_ne!(encrypted.bytes, zerocopy::AsBytes::as_bytes(&header));

		// Test header decryption.
		let decrypted = encrypted.decrypt(&header_key)?;
