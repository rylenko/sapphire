...
impl Kdf {
	...
	/// Expands KDF using passed `key`.
	///
	/// Use accessors to get expanded data.
	pub fn expand(&mut self, key: &[u8]) {
		KdfInner::new(Some(Self::SALT), key)
			.expand(Self::INFO, &mut self.0)
			.expect("Output must have a good length.");
	}

	/// Gets expanded authentication key.
	#[inline]
	#[must_use]
